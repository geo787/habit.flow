      ...
      const poll = async () => {
        try {
          ...
          if (data.payment_status === "paid") {
            ...
            toast.success(t("dashboard.proBannerTitle"));
            return;
          }
        } catch {}
    ...
    <div className="p-6 md:p-12 max-w-6xl">
      ...
      {showProBanner && (
        <div data-testid="pro-banner" className="ff-card p-5 mb-6 bg-gradient-to-r from-[#FFD166]/15 to-[#B19CD9]/15 ring-1 ring-[#FFD166] flex items-center gap-3">
          ...
          <div className="flex-1">
            <div className="font-extrabold">{t("dashboard.proBannerTitle")}</div>
            <div className="text-sm text-[#D0C7DB]">{t("dashboard.proBannerBody")}</div>
          </div>
      ...
      <div className="flex items-center justify-between mb-8 flex-wrap gap-4">
        ...
        <div className="ff-card px-5 py-3 flex items-center gap-3" data-testid="streak-badge">
          ...
          <div>
            <div className="text-xs text-[#8D829B]">{t("dashboard.streak")}</div>
            <div className="font-extrabold">{user?.streak || 0} {t("dashboard.days")}</div>
          </div>
        </div>
      </div>
      ...
      <div className="ff-card p-6 mb-8" data-testid="level-card">
        <div className="flex items-center justify-between flex-wrap gap-3 mb-3">
          <div className="flex items-center gap-3">
            <Sparkles className="text-[#FFD166]" />
            <div>
              <div className="text-xs text-[#8D829B] uppercase tracking-widest">{t("dashboard.level")} {lvl?.level}</div>
              <div className="font-extrabold text-xl">{lvl?.title}</div>
            </div>
          </div>
          <div className="text-sm text-[#D0C7DB]">{user?.xp} / {lvl?.next_threshold} {t("common.xp")}</div>
        </div>
        <div className="h-3 bg-[#1A1625] rounded-full overflow-hidden">
          <div className="h-full bg-gradient-to-r from-[#FFD166] via-[#B19CD9] to-[#45B69C] transition-all"
            style={{ width: `${Math.min(100, (lvl?.progress || 0) * 100)}%` }} />
        </div>
      </div>

      <div className="grid lg:grid-cols-3 gap-6 mb-6">
        <button data-testid="start-focus-btn" onClick={() => navigate("/focus")} className="lg:col-span-2 ff-card p-10 text-left relative overflow-hidden hover:bg-[#352D47] transition-all">
          <div className="absolute -top-10 -right-10 h-56 w-56 rounded-full bg-[#FFD166]/15 blur-3xl ff-pulse-soft" />
          <div className="relative">
            <div className="text-xs uppercase tracking-[0.2em] text-[#FFD166]">{t("dashboard.primary")}</div>
            <h2 className="text-3xl sm:text-4xl font-black mt-2 mb-3">{t("dashboard.startSprintTitle")}</h2>
            <p className="text-[#D0C7DB] mb-6 max-w-md">{t("dashboard.startSprintBody", { minutes: user?.focus_length || 15 })}</p>
            {usage && !usage.is_pro && (
              <div className="text-xs text-[#8D829B] mb-3" data-testid="free-session-counter">
                <Lock className="inline mr-1" size={11} /> {t("dashboard.sessionCounter", { used: usage.used, limit: usage.limit })} · <Link to="/pricing" className="text-[#FFD166] font-bold">{t("dashboard.upgrade")}</Link>
              </div>
            )}
        ...
        <div className="ff-card p-6 lg:col-span-2" data-testid="today-tasks">
          ...
          {todayTasks.length === 0 ? (
            ...
            <ul className="space-y-3">
              {todayTasks.map((tk) => (
                <li key={tk.id} data-testid={`dash-task-${tk.id}`} className="flex items-center gap-3 p-3 rounded-2xl bg-[#1A1625]/60 border border-[#3A3249]/40">
                  <div className="text-2xl">{tk.emoji_tag || "📝"}</div>
                  <div className="flex-1 truncate">{tk.title}</div>
                </li>
              ))}
            </ul>
          )}
        ...
        <div className="space-y-6">
          <div className="ff-card p-6">
            <div className="flex items-center gap-2 mb-3">
              ...
              <h3 className="font-extrabold">{t("dashboard.howAreYou")}</h3>
            </div>
            {!moodOpen ? (
              <button data-testid="open-mood-btn" onClick={() => setMoodOpen(true)} className="ff-btn-ghost w-full">{t("dashboard.tenSecCheckin")}</button>
            ) : (
              <div className="space-y-4">
                <div>
                  <div className="text-xs text-[#8D829B] mb-1">{t("dashboard.mood")}</div>
                  <div className="flex justify-between gap-1">
                    {moodEmojis.map((e, i) => (
                      <button key={i} data-testid={`mood-${i + 1}`} onClick={() => setMood(i + 1)} className={`text-2xl p-2 rounded-xl ${mood === i + 1 ? "bg-[#FFD166]/20 ring-2 ring-[#FFD166]" : ""}`}>{e}</button>
                    ))}
                  </div>
                </div>
                <div>
                  <div className="text-xs text-[#8D829B] mb-1">{t("dashboard.energy")}</div>
                  <input data-testid="energy-slider" type="range" min="1" max="5" value={energy} onChange={(e) => setEnergy(Number(e.target.value))} className="w-full accent-[#FFD166]" />
                </div>
                <button data-testid="submit-mood-btn" onClick={submitMood} className="ff-btn-primary w-full">{t("dashboard.saveMood")}</button>
              </div>
            )}
          </div>
          ...
          <Link to="/body-double" data-testid="body-double-pulse" className="ff-card p-6 block hover:bg-[#352D47]">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="font-extrabold">{t("dashboard.bodyDoubling")}</span>
            </div>
      ...
      >
        <CloudRain className="text-[#E07A5F]" size={18} />
        <span className="font-bold text-[#E07A5F]">{t("dashboard.overwhelmButton")}</span>
      </button>
    </div>
  );
}
